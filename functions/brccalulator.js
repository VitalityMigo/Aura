...
async function runesProfitMultiple(cont, walls, time) {
    ...
    try {
        ...
        const btcPrice = getBtcPrice()
        const tokenPRM = getRuneMetrics(slug, btcPrice)
        const heldPRM = getRuneCumulativeBalance(slug, wallets)
                ...
                if (item.action == 'buying_broadcasted') {
                        ...
                        if (isBlaster) {
                            ...
                            if (receivers < 5) {
                                ...
                                if (senders === 1) {
                                    ...
                                    // Pour ces raisons on ajoute les datas qui vont avec. Il faut aussi qu'on définisse la valeu.
                                    // On calcul la valeur qui sort du premier wallet qui n'est pas un wallet taproot ou du premier qui est notre wallet
                                    const filteredIn = inflow.filter(i => !isBRC20BitcoinWallet(i.prevout.scriptpubkey_address) || i.prevout.scriptpubkey_address === item.wallet)
                    ...
                    if (!isHiddenBuy) {
                        ...
                        if (isBlaster) {
                            ...
                            const inflow = txn.vin
                            const outflow = txn.vout // On utilise le flux pour déterminer si c'est un mint ou un airdrop
                            const receivers = [...new Set(outflow.filter(i => i.scriptpubkey_address === item.wallet).map(i => i.scriptpubkey_address))]

                            // On commence la dernière arborécance
                            if (!receivers.includes(item.wallet)) {
                                // Si le receiver est pas dans les wallet, ça veut dire
                                // que c'est un achat caché donc on le compte comme tel.
                                // On calcul la valeur reçu par le user. Elle n'est pas précise !!!!
                                const value = inflow.find(i => !isBRC20BitcoinWallet(i.prevout.scriptpubkey_address) && i.prevout.scriptpubkey_address != item.wallet).prevout.value
                                ...
                                // On vérifie que la subtransaction a bien uniquement des UTXO, sinon c'est que  
                                // c'est un sell avec wallet Unisat.
                                const filtered = outflow.filter(i => i.scriptpubkey_address === item.wallet)
                                const isUTXOOnly = isRunesUtxo(filtered, btcPrice)
