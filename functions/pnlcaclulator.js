...
async function coinProfitMultipleWallet(cont, wall, time) {
    ...
    try {
        ...
        for (const wallet of wallets) {
            ...
            // c'est la dernière étape de la boucle (hors approval)
            const [heldRaw] = await Promise.all([heldCALL]);
            data.heldAmount += heldRaw / 10 ** decimals
        ...
        // On récupère les valeurs CALL au début
        // On calcul quelques valeurs en plus
        const [priceRaw, supply] = await Promise.all([priceCALL, supplyCALL]);
        const priceUSD = priceRaw.priceUSD
        const priceETH = priceRaw.priceETH
...
async function nftProfitMulitWallet(cont, wall, time) {
...
module.exports = {
    ...
    nftProfitSingle,
    nftProfitGlobal,
    nftProfitMulitWallet,
    coinProfitMultipleWallet
}
