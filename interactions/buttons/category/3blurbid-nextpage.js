        ...
        try {
            ...
            if (interaction.message.interaction.user.id === authorId) {
                ...
                if (commandCategory == "collection") {
                    ...
                    if (newPage == pageIndex) { await interaction.update({ embeds: [getBlurOneWallet], components: [buttonsRow] }); }
                    else { await interaction.update({ embeds: [getBlurOneWallet], components: [buttonsRowAllGood] }); }


                    await interactionData.update({ actualPage: newPage.toString(), }, { where: { authorId: authorId, commandName: "blur-bid", serverId: serverId } })
