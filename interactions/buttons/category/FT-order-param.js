...
module.exports = {
    id: 'button-friendtechtasksinfra-order-param-',
        ...
        try {
            ...
            if (interaction.message.interaction.user.id === authorId) {
                ...
                if (action == "delete") {
                    ...
                    let component2 = interaction.message.components[interaction.message.components.length - 1]
                    let componentBonus = ""

                    if (interaction.message.components.length == 3) {

                        componentBonus = interaction.message.components[1]
                    ...
                    if (taskEmbed.fields.find(obj => obj.name === "Status").value == "`🟢 Active`") {
                        ...
                        component2.components.find(obj => obj.data.label === "🔴 Disable").data.label = "🟢 Activate"




                        await order_friendTech.update({ active: "false", }, { where: { authorId: authorId, randomId: uniqueId } });

                        if (componentBonus == "") {
                            await interaction.update({ embeds: [taskEmbed], components: [component1, component2], ephemeral: true });
                        } else {

                            await interaction.update({ embeds: [taskEmbed], components: [component1, componentBonus, component2], ephemeral: true });
                        ...
                        if (userSetup != null) {
                            ...
                            const belowPrice = taskDetails.dataValues.min_key_price
                            const abovePrice = taskDetails.dataValues.max_key_price
                            const target = taskDetails.dataValues.target


                            if (target != null && (belowPrice != null || abovePrice != null)) {
                                ...
                                let component2 = interaction.message.components[interaction.message.components.length - 1]
                                let componentBonus = ""
                                console.log(component2.components)
                                ...
                                component2.components.find(obj => obj.data.label === "🟢 Activate").data.label = "🔴 Disable"




                                await order_friendTech.update({ active: "true", }, { where: { authorId: authorId, randomId: uniqueId } });
                                ...
                                const gasTrackerEmbed = new EmbedBuilder().setColor("#060A8F")
                                    ...
                                    .setDescription("You can't activate your task unless you set few settings before, including a target (Friend.Tech subject) and either a *Below price* or *Above price*. Please try again after doing so.")
                                    .setAuthor({ name: authorName, iconURL: userAvatar })
                                    .setTimestamp()
                                    .setFooter({ text: 'Powered by Rolls Chasers', iconURL: 'https://cdn.discordapp.com/attachments/1108757872315219968/1121978623436521514/rc_logo.png' })
                    ...
                    const passwordAdminDashboard = new ModalBuilder()
                        .setCustomId('modal-friendtechtasksinfra-order-param-amount@' + uniqueId)
                        .setTitle('Set Amount');
                    ...
                    const channel = new TextInputBuilder()
                        .setCustomId('modal-friendtechtasksinfra-order-param-amount@' + uniqueId + 'R1')
                        .setLabel("Amount")
                        .setPlaceholder("The amount of token to buy/sell per user")
                    ...
                    const passwordAdminDashboard = new ModalBuilder()
                        .setCustomId('modal-friendtechtasksinfra-order-param-target@' + uniqueId)
                        .setTitle('Set Target');
                    ...
                    // Create the text input components
                    const channel = new TextInputBuilder()
                        .setCustomId('modal-friendtechtasksinfra-order-param-target@' + uniqueId + 'R1')
                        .setLabel("Target Username")
                        .setPlaceholder("The Twitter username of the new user to buy/sell")
                        .setStyle(TextInputStyle.Short)
                        .setRequired(true)
                    ...
                    const passwordAdminDashboard = new ModalBuilder()
                        .setCustomId('modal-friendtechtasksinfra-order-param-totaltask@' + uniqueId)
                    ...
                    // Create the text input components
                    const channel = new TextInputBuilder()
                        .setCustomId('modal-friendtechtasksinfra-order-param-totaltask@' + uniqueId + 'R1')
                        .setLabel("Amount")
                        .setPlaceholder("The amount of order the bot execute before stopping")
                        .setStyle(TextInputStyle.Short)
                        .setRequired(false)
                    ...
                    const passwordAdminDashboard = new ModalBuilder()
                        .setCustomId('modal-friendtechtasksinfra-order-param-price@' + uniqueId)
                        .setTitle('Set Price');
                    ...
                    // Create the text input components
                    const channel = new TextInputBuilder()
                        .setCustomId('modal-friendtechtasksinfra-order-param-price@' + uniqueId + 'R1')
                        .setLabel("Buy/Sell Below")
                        .setPlaceholder("The value at which the bot should buy/sell if it falls below it")
                        .setStyle(TextInputStyle.Short)
                        .setRequired(false)

                    const channel2 = new TextInputBuilder()
                        .setCustomId('modal-friendtechtasksinfra-order-param-price@' + uniqueId + 'R2')
                        .setLabel("Buy/Sell Above")
                        .setPlaceholder("The value at which the bot should buy/sell if it is exceded")
                        .setStyle(TextInputStyle.Short)
                        .setRequired(false)
                    ...
                    const passwordAdminDashboard = new ModalBuilder()
                        .setCustomId('modal-friendtechtasksinfra-order-param-gaspreset@' + uniqueId)
                    ...
                    // Create the text input components
                    const channel = new TextInputBuilder()
                        .setCustomId('modal-friendtechtasksinfra-order-param-gaspreset@' + uniqueId + 'R1')
                        .setLabel("Gas Ratio")
...
console.log(uniqueId)
                    ...
                    if (taskEmbed.fields.find(obj => obj.name === "Simulation").value == "`✅`") {
                        ...
                        await order_friendTech.update({ simulation: "false", }, { where: { authorId: authorId, randomId: uniqueId } });
                        ...
                        await order_friendTech.update({ simulation: "true", }, { where: { authorId: authorId, randomId: uniqueId } });
                        await interaction.update({ embeds: [taskEmbed], ephemeral: true });
