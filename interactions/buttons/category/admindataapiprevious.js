...
const { accessSql, profileData, interactionData, apiproviderssql, apimonitorsql, adminsql, reportsql, sequelize } = require('../../../events/database');
...
const buttonRowAdminDashboard = new ActionRowBuilder()
    .addComponents(
        new ButtonBuilder()
            .setCustomId('adminDataApiCurrentMonth-button')
...
module.exports = {
    id: 'adminDataApiPrevMonth-button',
    ...
    async execute(interaction) {
        ...
        if (interaction.message.interaction.user.id === authorId) {
            ...
            const apiMonthReportInteractions = await interactionData.findOne({ where: { authorId: "bot", commandName: "intervalApiMonth", serverId: "bot" } })

            if (apiMonthReportInteractions !== null) {

                const lastMonthReportApi = JSON.parse(apiMonthReportInteractions.dataValues.embed1)

                await interaction.update({ embeds: [lastMonthReportApi], components: [buttonRowAdminDashboard] })
            ...
            let reportCommand = "/admin-dataApiPrevMonth"
