...
module.exports = {
    id: 'teamSubCancelSubConfirm-button',
    ...
    async execute(interaction) {
        ...
        if (interaction.message.interaction.user.id === authorId) {
            ...
            const passwordManagement = new EmbedBuilder().setColor("#060A8F")
                ...
                .addFields(
                    ...
                    { name: "Statut", value: "Your subscribtion has been canceled, the bot is deleting your datas from the database. The bot will leave the server shortly.\n\nThe Rolls Chasers Analytics Team thanks you for trusting us. We hope to see you back soon !", inline: false },
            ...
            const updateEmbed = new EmbedBuilder().setColor("#FF2D00")
                .setTitle("Subscribtion Canceled")
                .setDescription(">>> A community just canceled its subscribtion.")
                .setThumbnail('https://media.discordapp.net/attachments/949300412874362983/1040242440696758282/Logo_Rolls_V2_5.3_auto_x2.jpg')
                .setAuthor({ name: authorName, iconURL: userAvatar })
                .setTimestamp()
                .addFields(
                    { name: " ", value: " ", inline: false },
                    { name: "Content:", value: "The community named `" + serverName + "` just canceled their subscribtion. The bot `leaved`the server and the database has been cleared accordingly.", inline: false },
                )
                .setFooter({ text: 'Powered by Rolls Chasers', iconURL: 'https://cdn.discordapp.com/attachments/1108757872315219968/1121978623436521514/rc_logo.png' })
            ...
            //On clear les databases de commu (ajouter alerts quand merge)
            await interactionData.destroy(({ where: { serverId: serverId } }))
            await walletsgenerated.destroy(({ where: { serverId: serverId } }))
            await vouchData.destroy(({ where: { serverId: serverId } }))
            await accessSql.destroy(({ where: { serverId: serverId } }))

            //Le bot quitte le serveur
            const actualGuild = interaction.guild;
        ...
        let reportCommand = "/admin-subscribtionCancelConfirm"
