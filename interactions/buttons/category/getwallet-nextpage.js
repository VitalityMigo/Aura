...
const buttonLastPage = new ActionRowBuilder()
.addComponents(
    new ButtonBuilder()
        .setCustomId('zzzgetwalletfirstpage-button')
        .setLabel('first page')
...
const buttonLastPage1 = new ActionRowBuilder()
.addComponents(
    new ButtonBuilder()
        .setCustomId('zzzgetwalletfirstpage-button')
        .setLabel('first page')
...
module.exports = {
  id: 'zzzgetwalletnextpage-button',
  ...
  async execute(interaction, message) {
    ...
    if (interaction.message.interaction.user.id === authorId) {
      ...
      const lastInteraction = await interactionData.findOne({ where: { authorId: authorId, commandName: "getwallet", serverId: serverId } })
                ...
                const futurePage = parseInt(actualPage) + 1
                ...
                if (selectedChain.toLowerCase() === "eth") {
                    ...
                    //console.log(filteredByWalletETH)
                    let walletEthSliced = [] 
                    if (futurePage == 2){ walletEthSliced = filteredByWalletETH.slice(8, 16) } else if (futurePage == 3){ walletEthSliced = filteredByWalletETH.slice(16, 24); }
...
embed1Field += "`" + (object.walletAddress).substring(0, 8) + "…" + (object.walletAddress).substring((object.walletAddress).length - 9, (object.walletAddress).length) + spaceLenght + (object.walletName).toLowerCase() + spaceLenght2 + object.balance + sign + "`\n"
                                ...
                                if (fieldCount == 7 || fieldCount == walletStop) {
                                    ...
                                    getwalletsAllEmbed.addFields(
                                        { name: 'Page', value: "`[" + futurePage + "/" + pageIndex + "]`", inline: false },
                                    )
                    ...
                    if (walletShow == "no") {
                        ...
                        if (futurePage < pageIndex) {

                            await interaction.update({ embeds: [getwalletsAllEmbed], components: [buttonAllAvailable, buttonShowName] })
    
                        } else if (futurePage == pageIndex) {
    
                            await interaction.update({ embeds: [getwalletsAllEmbed], components: [buttonLastPage, buttonShowName] })
                        ...
                        if (futurePage < pageIndex) {
                            ...
                            await interaction.update({ embeds: [getwalletsAllEmbed], components: [buttonLastPage, buttonShowName1] })
                    ...
                    await interactionData.update({ actualPage: futurePage.toString(), }, { where: { commandName: "getwallet", authorId: authorId, serverId: serverId } })
...
embed1Field += "`" + (object.walletAddress).substring(0, 8) + "…" + (object.walletAddress).substring((object.walletAddress).length - 9, (object.walletAddress).length) + spaceLenght + (object.walletName).toLowerCase() + spaceLenght2 + object.balance + sign + "`\n"
    ...
    let reportCommand = "/getwallet-nextPage"
