  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (interaction.message.interaction.user.id === authorId) {
        ...
        const lastInteractionRcprofit = await interactionData.findOne({ where: { authorId: authorId, commandName: "runes-profit", serverId: serverId } })

        // On récupère les data principales
        const userName = lastInteractionRcprofit.dataValues.authorName
        const userLogo = lastInteractionRcprofit.dataValues.userAvatar

        // JSON
        const record = JSON.parse(lastInteractionRcprofit.dataValues.embed1)
        ...
        if (serverId === "949291624389816331") {
          ...
          if (visualSelect === "2" || privacyBigDataAuthor === null) {
            ...
            ctx.font = "bold 22px Futura";
            ctx.fillStyle = "#ffffff";
            ctx.fillText((parseFloat((remaining * floorPrice).toFixed(3))).toString() + "B", 337, 548.5);


            //REALIZED PROFIT
            ctx.font = "bold 22px Futura";
            ctx.fillStyle = "#ffffff";
            ctx.fillText(parseFloat(realisedProfit).toFixed(3) + "B", 337, 594);
            ...
            ctx.fillStyle = "#ffffff";
            const text = parseFloat(potentialProfit).toFixed(3) + "B" // (" + Intl.NumberFormat('en-US').format((parseFloat(potentialProfit * btcPrice).toFixed(0))) + "$)";
            ctx.font = "bold 35px Futura";
            const text2 = "(" + Intl.NumberFormat('en-US').format((parseFloat(potentialProfit * btcPrice).toFixed(0))) + "$)";
            ...
            ctx.font = "bold 28px Futura";
            ctx.fillStyle = "#ffffff";
            ctx.fillText(parseFloat(avgBuy).toFixed(3) + "B", 343, 823);

            //AVG SALE VALUE
            ctx.font = "bold 28px Futura";
            ctx.fillStyle = "#ffffff";
            ctx.fillText(parseFloat(avgSold).toFixed(3) + "B", 343, 872);
            ...
            ctx.font = "bold 28px Futura";
            ctx.fillStyle = "#ffffff";
            ctx.fillText((parseFloat(remaining * floorPrice).toFixed(3)).toString() + "B", 840, 823);

            //REALIZED PROFIT
            ctx.font = "bold 28px Futura";
            ctx.fillStyle = "#ffffff";
            ctx.fillText(parseFloat(realisedProfit).toFixed(3) + "B", 840, 872);
            ...
            if (potentialProfit >= 0) { ctx.fillStyle = "#00db00"; } else if (potentialProfit < 0) { ctx.fillStyle = "#e60015"; }
            ctx.fillText("$" + Intl.NumberFormat('en-US').format((parseFloat(potentialProfit * btcPrice).toFixed(0))), 45, 653);

            //POTENTIAL PROFIT (ETH)
            ctx.font = "bold 80px SFTransrobotic";
            if (potentialProfit >= 0) { ctx.fillStyle = "#00db00"; } else if (potentialProfit < 0) { ctx.fillStyle = "#e60015"; }
            ctx.fillText(parseFloat(potentialProfit).toFixed(3) + "B", 547, 653);
            ...
            ctx.font = "bold 31px Futura";
            ctx.fillStyle = "#E5EAFF";
            const avgBuyCountText = ctx.measureText((parseFloat(avgBuy).toFixed(3)).toString() + "B").width
            ctx.fillText((parseFloat(avgBuy).toFixed(3)).toString() + "B", 795 - avgBuyCountText / 2, 420);
            ...
            ctx.font = "bold 31px Futura";
            ctx.fillStyle = "#E5EAFF";
            const avgSoldText = ctx.measureText((parseFloat(avgSold).toFixed(3)).toString() + "B").width
            ctx.fillText((parseFloat(avgSold).toFixed(3)).toString() + "B", 795 - avgSoldText / 2, 585);


            //REALIZED PROFIT
            ctx.font = "bold 31px Futura";
            if (realisedProfit >= 0) { ctx.fillStyle = "#00db00"; } else if (potentialProfit < 0) { ctx.fillStyle = "#e60015"; }
            const realisedProfitText = ctx.measureText((parseFloat(realisedProfit).toFixed(3)).toString() + "B").width
            ctx.fillText((parseFloat(realisedProfit).toFixed(3)).toString() + "B", 201 - realisedProfitText / 2, 749);

            //Potential PROFIT
            ctx.font = "bold 31px Futura";
            if (potentialProfit >= 0) { ctx.fillStyle = "#00db00"; } else if (potentialProfit < 0) { ctx.fillStyle = "#e60015"; }
            const potentialProfitText = ctx.measureText((parseFloat(potentialProfit).toFixed(3)).toString() + "B").width
            ctx.fillText((parseFloat(potentialProfit).toFixed(3)).toString() + "B", 498 - potentialProfitText / 2, 749);
          ...
          ctx.font = "bold 38px Courrier New";
          ctx.fillStyle = "#ffffff";
          const avgSpentTextSize = ctx.measureText(parseFloat(avgBuy).toFixed(3) + "B").width;
          ctx.fillText(parseFloat(avgBuy).toFixed(3) + "B", 222 - avgSpentTextSize / 2, 740);

          //AVG SALE VALUE
          ctx.font = "bold 38px Courrier New";
          ctx.fillStyle = "#ffffff";
          const avgSaleTextSize = ctx.measureText(parseFloat(avgSold).toFixed(3) + "B").width;
          ctx.fillText(parseFloat(avgSold).toFixed(3) + "B", 504 - avgSaleTextSize / 2, 740);


          //Realized PROFIT (ETH)
          ctx.font = "bold 38px Courrier New";
          //if (realisedProfit >= 0) { ctx.fillStyle = "#00db00"; } else if (realisedProfit < 0) { ctx.fillStyle = "#e60015"; }
          const realizedProfitTextSize = ctx.measureText(parseFloat(realisedProfit).toFixed(3) + "B").width;
          ctx.fillText(parseFloat(realisedProfit).toFixed(3) + "B", 803 - realizedProfitTextSize / 2, 740);
          ...
          if (potentialProfit >= 0) { ctx.fillStyle = "#00db00"; } else if (potentialProfit < 0) { ctx.fillStyle = "#e60015"; }
          const potentialProfitTextSize = ctx.measureText(parseFloat(potentialProfit).toFixed(3) + "B (" + potentialRoiFormatted + "%)").width;
          ctx.fillText(parseFloat(potentialProfit).toFixed(3) + "B (" + potentialRoiFormatted + "%)", 500 - potentialProfitTextSize / 2, 876);
          ...
          ctx.font = "38px rbt";
          ctx.fillStyle = "#04D9FF";
          const usdTx = parseFloat(potentialProfit * btcPrice).toFixed(2) + " USD"
          ...
          // Cout total
          // ON commence par formatter le texte
          const spentTx = parseFloat(totalBuy).toFixed(3) + "B"
          // Puis on met la font et la couleur
          ...
          // Sales total
          // ON commence par formatter le texte
          const salesTx = parseFloat(totalSell).toFixed(3) + "B"
          // Puis on met la font et la couleur
          ctx.font = "light 19px rbt";
          ctx.fillStyle = "#E7E7E7";
          // Enfin on calcul la taille et remplit
          ...
          const buyTXT = "$" + formatDollars(totalBuy * btcPrice)
          ctx.font = " 50px 'UTM Futura Extra'";
          ctx.fillStyle = "#828282";
          ctx.fillText(buyTXT, 658, 496);

          const profitUsdTXT = "$" + formatDollars(potentialProfit * btcPrice)
          ctx.font = " 50px 'UTM Futura Extra'";
          ctx.fillStyle = "#828282";
          ...
          ctx.font = "700 35px 'Fira Code'";
          ctx.fillStyle = "#E5EAFF";
          const avgMCBuy = formatNumberVisual(data.avgMCBuy).toString() + "$"
          const avgBuyCountText = ctx.measureText(avgMCBuy).width
          ctx.fillText(avgMCBuy, 812 - avgBuyCountText / 2, 420);

          //SOLD COUNT
          ctx.font = "700 35px 'Fira Code'";
          ctx.fillStyle = "#E5EAFF";
          const sellAmount = formatNumberVisual(data.sellAmount).toString()
          const sellAmountText = ctx.measureText(sellAmount).width
          ctx.fillText(sellAmount, 190 - sellAmountText / 2, 585);

          //REMAINING
          ctx.font = "700 35px 'Fira Code'";
          ctx.fillStyle = "#E5EAFF";
          const heldAmount = formatNumberVisual(data.heldAmount).toString()
          const heldAmountText = ctx.measureText(heldAmount).width
          ctx.fillText(heldAmount, 500 - heldAmountText / 2, 585);

          //AVG SOLD
          ctx.font = "700 35px 'Fira Code'";
          ctx.fillStyle = "#E5EAFF";
          const avgMCSell = formatNumberVisual(data.avgMCSell).toString() + "$"
          const avgMCSellText = ctx.measureText(avgMCSell).width
          ctx.fillText(avgMCSell, 812 - avgMCSellText / 2, 585);
