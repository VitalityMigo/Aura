...
const { accessSql, profileData, adminsql, reportsql, walletManager, sequelize } = require('../../../events/database');
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const buttons1Row = new ActionRowBuilder()
            .addComponents(
                new ButtonBuilder()
                    .setCustomId('walletmanagersetupsetmain-button')
                    .setLabel('set main wallet')
                    .setStyle(3),
                new ButtonBuilder()
        ...
        const mainWalletAuthorInfos = await walletManager.findOne({ where: { authorId: authorId } })

        if (mainWalletAuthorInfos !== null) {

            let mainPublic = mainWalletAuthorInfos.dataValues.walletAddress
            let mainPrivate = mainWalletAuthorInfos.dataValues.walletPK

            mainPublic = decrypt(mainPublic);
            mainPrivate = decrypt(mainPrivate);



            const ethBalance = await web3.eth.getBalance(mainPublic)
            ...
            sdk.getUsersActivityV6({ users: mainPublic, accept: '*/*' })
                .then(async ({ data: userActivity }) => {
                    ...
                    for (const txn of userActivity.activities) {
                        ...
                        txnType = txn.type
                        if (txn.fromAddress == mainPublic && txn.toAddress == mainPublic) { txnDirection = "out" }
                    ...
                    const walletManagerEmbed = new EmbedBuilder().setColor("#060A8F")
                        ...
                        .addFields(
                            ...
                            { name: "Public address", value: "`" + mainPublic + "`", inline: true },
                            { name: "Private Key", value: "`" + mainPrivate + "`", inline: true },
                            { name: " ", value: " ", inline: false },
            ...
            const walletManager = new EmbedBuilder().setColor("#060A8F")
                ...
                .addFields(
                    { name: " ", value: " ", inline: false },
                    { name: "Public address", value: "`no main`", inline: true },
                    { name: "Private Key", value: "`no main`", inline: true },
                    { name: " ", value: " ", inline: false },
                    { name: "Available Funds", value: "`no fund`", inline: true },
                    { name: "Txn History", value: "`no main`", inline: false },
                    { name: " ", value: " ", inline: false },
                    { name: " ", value: "*⚠️ Please note that your main wallet's private key is encrypted with web3 protocol.*", inline: false },
            ...
            await interaction.update({ embeds: [walletManager], components: [buttons1Row] });
