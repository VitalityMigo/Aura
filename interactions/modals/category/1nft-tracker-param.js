...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const match = customId.match(/modal_nft_infra_tracker_(.+)/);
            ...
            if (match && match[1]) {
                ...
                if (action == "add") {
                    ...
                    // On formatte l'emebed en attendant
                    let taskEmbed = interaction.message.embeds[0].data
                    taskEmbed.fields.push(
                        { value: ' ', name: ' ', inline: false },
                        { name: ' ', value: 'Adding the address to your NFT wallet tracker, please stand by <a:AuraLoading:1134068847616458792>', inline: false }
                    )
                    await interaction.editReply({ embeds: [taskEmbed], ephemeral: true });


                    const addresses = interaction.fields.getTextInputValue('modal_nft_infra_tracker_' + action + 'R1');
                    ...
                    if (userList.length < 15) {
                        ...
                        if (addressCount > 0) {
                            ...
                            // On formatte et envoi les emebed
                            let baseAdd = ""
                            if (registeredList.length > 0) {
                                baseAdd = taskEmbed.fields.find(obj => obj.name === "Address Tracked:").value.replace(/```/g, "");
                            }

                            const list = addressString.join('\n');
                            if (confirmed > 0) {
                                taskEmbed.fields.find(obj => obj.name === "Address Tracked:").value = "```\n" + baseAdd + "\n" + list + "                                                 ```"
                                taskEmbed.fields.find(obj => obj.name === "Spots Left").value =  "`" + (parseInt(taskEmbed.fields.find(obj => obj.name === "Spots Left").value.replace(/`/g, "")) - confirmed) +  "`"
                                taskEmbed.fields.find(obj => obj.name === "Address Count").value =  "`" + (parseInt(taskEmbed.fields.find(obj => obj.name === "Address Count").value.replace(/`/g, "")) + confirmed) +  "`"
                            }
                            ...
                            if (confirmed > 0) {
                                ...
                                const setfpEmbedNotForYou = new EmbedBuilder().setColor("#060A8F")
                                    ...
                                    .setDescription("Added `" + confirmed + "` address(es) to your NFT wallet tracker. The bot is now monitoring them ✅")
                                ...
                                const setfpEmbedNotForYou = new EmbedBuilder().setColor("#060A8F")
                                    .setTitle("NFT Tracker")
                                    .setAuthor({ name: authorName, iconURL: userAvatar })
                                    .setDescription("No new wallet added. This can happen if the wallet is already registered in your NFT wallet tracker ❌")
                                    .setThumbnail('https://cdn.discordapp.com/attachments/1108757847208099941/1133190291428479016/image.png')
                                    .setTimestamp()
                                    .setFooter({ text: 'Powered by Rolls Chasers', iconURL: 'https://cdn.discordapp.com/attachments/1108757872315219968/1121978623436521514/rc_logo.png' })
                    ...
                    if (addressCount > 0) {
                        ...
                        if (registeredList.includes(add)) {
                            ...
                            await tracker_nft.destroy({ where: { authorId: authorId, address: add } })
                            ...
                            const list = registeredList.filter(item => item != add).join('\n');
                            if (registeredList.length - 1 == 0) {
                                taskEmbed.fields.find(obj => obj.name === "Address Tracked:").value = "```No tracked address found in your profile                ```"
                            } else {
                                taskEmbed.fields.find(obj => obj.name === "Address Tracked:").value = "```\n" + list + "                   ```"
                            }
