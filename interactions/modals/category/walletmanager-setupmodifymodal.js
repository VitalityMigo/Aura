...
module.exports = {
    id: 'walletmanagersetupmodifymain-modal',
    ...
    async execute(interaction) {
        ...
        const walletPublic = interaction.fields.getTextInputValue('publicKeyModify');
        const walletPrivate = interaction.fields.getTextInputValue('privateKeyModify');

        const encryptedAddress = encrypt(walletPublic);
        const encryptedPK = encrypt(walletPrivate);


        await walletManager.update({ walletAddress: encryptedAddress.toString(), walletPK: encryptedPK.toString() }, { where: { authorId: authorId } })



        //On récupère la balance de token

        const ethBalance = await web3.eth.getBalance(walletPublic)
        ...
        sdk.getUsersActivityV6({ users: walletPublic, accept: '*/*' })
            .then(async ({ data: userActivity }) => {
                ...
                for (const txn of userActivity.activities) {
                    ...
                    txnType = txn.type
                    console.log(txnType)
                    if (txn.fromAddress == walletPublic && txn.toAddress == walletPublic) { txnDirection = "out" }
                ...
                const walletManagerEmbed = new EmbedBuilder().setColor("#060A8F")
                    ...
                    .addFields(
                        ...
                        { name: "Public address", value: "`" + walletPublic + "`", inline: true },
                        { name: "Private Key", value: "`" + walletPrivate + "`", inline: true },
                        { name: " ", value: " ", inline: false },
