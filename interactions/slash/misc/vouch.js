...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option
                .setName("member")
                .setDescription("The member you want to vouch for")
                .setRequired(true)
        )
        .addStringOption(option =>
            option
                .setName("reason")
                .setDescription("The reason that pushed you to vouch for the member")
                .setRequired(true)
        ),
                    ...
                    if (communityStatut.toLowerCase() === "active" || communityStatut == "") {
                        ...
                        if (accessTier.toLowerCase() == "b-tier" || accessTier.toLowerCase() == "a-tier" || accessTier.toLowerCase() == "s-tier") {
                            ...
                            if (member.roles.cache.has(communityMemberRoleId)) {
                                ...
                                const selectedMember = interaction.options.getString("member");
                                const vouchReason = interaction.options.getString("reason");

                                //Récupérer le timestamp actuel
                                const timeStamp = Date.now();
                                const actualTimestamp = parseFloat(timeStamp / 1000).toFixed(0)
                                const vouchDelayTimestamp = actualTimestamp - 86400


                                const authorVouches = await vouchData.findAll({ where: { authorId: authorId, serverId: serverId } })
                                const memberVoucheScore = await vouchData.findAll({ where: { memberId: selectedMember, serverId: serverId } })
                                const voucheScore = memberVoucheScore.length + 1


                                const isMoreThan24Hours = authorVouches.filter((vouchData) => {
                                    return Number(vouchData.dataValues.vouchTimestamp) > vouchDelayTimestamp;
                                });



                                if (selectedMember !== ("<@" + authorId + ">")) {


                                    if (isMoreThan24Hours.length <= 0) {

                                        await vouchData.create({
                                            authorId: authorId,
                                            vouchTimestamp: actualTimestamp,
                                            vouchReason: vouchReason,
                                        ...
                                        const vouchEmbed = new EmbedBuilder().setColor("#060A8F")
                                            .setTitle(`${authorName}'s vouch`)
                                            .setDescription(">>> Display the vouch of infos " + authorName)
                                            .setThumbnail('https://cdn.discordapp.com/attachments/1108757847208099941/1133190291428479016/image.png')
                                            .setAuthor({ name: authorName, iconURL: userAvatar })
                                            .addFields(
                                                { name: "Member", value: selectedMember, inline: true },
                                                { name: "Member Score", value: "`" + voucheScore + " points`", inline: true },
                                                { name: "Reason", value: '"' + vouchReason + '"', inline: false },
                                    ...
                                    } else if (isMoreThan24Hours.length > 0) {


                                        const timestampsGrabber = isMoreThan24Hours.map(item => item.dataValues.vouchTimestamp);
                                        const lastVouch = timestampsGrabber.reduce((acc, curr) => Math.max(acc, curr), 0);
                                        const delayToVouch = 86400 - (actualTimestamp - lastVouch)

                                        function formatTime(delayToVouch) {
                                            const hours = Math.floor(delayToVouch / 3600);
                                            const minutes = Math.floor((delayToVouch % 3600) / 60);
                                        ...
                                        const vouchEmbedError = new EmbedBuilder().setColor("#060A8F")
                                            .setTitle(`${authorName}'s vouch`)
                                            .setDescription("You can vouch only once every 24 hours. Please try again in `" + formatTime(delayToVouch) + "`")
                                            .setAuthor({ name: authorName, iconURL: userAvatar })
                                            .setThumbnail('https://cdn.discordapp.com/attachments/1108757847208099941/1133190291428479016/image.png')
                                            .setTimestamp()
                                            .setFooter({ text: 'Powered by Rolls Chasers', iconURL: 'https://cdn.discordapp.com/attachments/1108757872315219968/1121978623436521514/rc_logo.png' });

                                        await interaction.editReply({ embeds: [vouchEmbedError] }, { ephemeral: true });


                                    }


                                } else if (selectedMember === ("<@" + authorId + ">")) {


                                    const vouchEmbedError = new EmbedBuilder().setColor("#060A8F")
                                        .setTitle(`${authorName}'s vouch`)
                                        .setDescription("You can't performe this action because you can't vouch for yourself. Try again vouching for a `community member` that is not you")
                                        .setAuthor({ name: authorName, iconURL: userAvatar })
                                        .setThumbnail('https://cdn.discordapp.com/attachments/1108757847208099941/1133190291428479016/image.png')
                                        .setTimestamp()
                                        .setFooter({ text: 'Powered by Rolls Chasers', iconURL: 'https://cdn.discordapp.com/attachments/1108757872315219968/1121978623436521514/rc_logo.png' });
